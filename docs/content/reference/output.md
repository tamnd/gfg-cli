...
describes what users get. Pick a format with `-o`, or let gfg choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
gfg <command> -o table   # aligned columns for reading
gfg <command> -o jsonl   # one JSON object per line, for piping
gfg <command> -o json    # a single JSON array
gfg <command> -o csv     # spreadsheet friendly
gfg <command> -o tsv     # tab-separated
gfg <command> -o url     # just the URL column
gfg <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
gfg <command> --fields id,title,url
```
...
```bash
gfg <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
gfg <command>            # a table, because this is a terminal
gfg <command> | wc -l    # JSONL, because this is a pipe
```
